...
export var Lemmatizer = function() {
  this.wn_files = {
    noun: [
      '../datas/variants/index.noun.json',
      '../datas/variants/noun.exc.json'
    ],
    verb: [
      '../datas/variants/index.verb.json',
      '../datas/variants/verb.exc.json'
    ],
    adj:  [
      '../datas/variants/index.adj.json',
      '../datas/variants/adj.exc.json'
    ],
    adv:  [
      '../datas/variants/index.adv.json',
      '../datas/variants/adv.exc.json'
    ]
  };
...
Lemmatizer.prototype = {
  ...
  init_store: async function(){
    ...
    if(!variants){
      let stores = []
      parts.forEach(p => {
        stores.push([p+"_idx",getdata(this.wn_files[p][0])])
        stores.push([p+"_exc",getdata(this.wn_files[p][1])])
      });
      await setVariants(stores)
    } else{
      const keys = Object.keys(variants)

      _.each(keys,(k)=>{
        if(k.includes("_idx")){
          self.wordlists[k.replace("_idx","")] = {}
          _.each(JSON.parse(variants[k]),(w)=>{
            self.wordlists[k.replace("_idx","")][w] = w
          })
        } else {
          self.exceptions[k.replace("_exc","")] = {}
          _.each(JSON.parse(variants[k]),(w)=>{
            self.exceptions[k.replace("_exc","")][w[0]] = w[1]
          })
        }
      })
    }
  ...
  open_file: async function(key, file) {
    ...
    if (!variants) {
      ...
      xhr.send();
      var data = xhr.responseText;
      stores.push([key,data])
    }
    await this.store_data(stores);
  },

  store_data: function(stores) {
    // localStorage.setItem(key, data);
    return setVariants(stores);
  },

  fetch_data: async function(key) {
    try{
      const variants = await getVariants(key)
      // var data = JSON.parse(localStorage.getItem(key));
      var data = JSON.parse(variants);
    } catch(e){
      console.log(e)
    }
    return data;
